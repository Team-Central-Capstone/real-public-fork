...
function Utils() {
  ...
  this.points = function (config) {
    const xs = this.numbers(config);
    const ys = this.numbers(config);
    return xs.map((x, i) => ({x, y: ys[i]}));
  }
  
  this.bubbles = function (config) {
  ...
  this.transparentize = function (value, opacity) {
  ...
  this.namedColor = function (index) {
    return NAMED_COLORS[index % NAMED_COLORS.length];
  }
  
  this.newDate = function (days) {
  ...
  this.parseISODate = function (str) {
